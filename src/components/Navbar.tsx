...
export default function Navbar() {
  ...
  return (
    <>
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? "bg-brand-bg/95 backdrop-blur-md border-b border-brand-border" : "bg-transparent"}`}>
        <div className="max-w-6xl mx-auto px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          {/* Mobile toggle */}
          <button onClick={() => setOpen(!open)} className="md:hidden text-brand-muted hover:text-brand-text">
      ...
      <div className={`fixed inset-0 z-40 md:hidden transition-all duration-300 ${open ? "opacity-100 visible" : "opacity-0 invisible"}`}>
        ...
        <div className={`absolute top-0 right-0 bottom-0 w-72 bg-brand-surface border-l border-brand-border transition-transform duration-300 ${open ? "translate-x-0" : "translate-x-full"}`}>
          <div className="flex items-center justify-between px-6 h-16 border-b border-brand-border">
            ...
            <button onClick={() => setOpen(false)}><X size={18} className="text-brand-muted" /></button>
          </div>
          <div className="p-6 flex flex-col gap-1">
            ...
            <div className="mt-6 pt-6 border-t border-brand-border flex flex-col gap-3">
              <a href={siteConfig.github} target="_blank" rel="noopener noreferrer"
                className="text-center py-2.5 border border-brand-border text-brand-text-dim text-sm rounded-sm hover:border-brand-border-light hover:text-brand-text transition-all">
                GitHub
              </a>
              <a href={`https://wa.me/${siteConfig.whatsapp}`} target="_blank" rel="noopener noreferrer"
