...
export default function DesignSection() {
  ...
  return (
    <section id="design" className="py-24 bg-brand-surface border-t border-brand-border-light">
          ...
          <div className="flex gap-2 flex-wrap">
            {categories.map(c => (
              <button key={c.value} onClick={() => setActive(c.value)}
                className={`px-4 py-1.5 text-xs tracking-wide border rounded-sm transition-all ${active === c.value ? "border-brand-text text-brand-text bg-brand-card" : "border-brand-border-light text-brand-text-dim hover:border-brand-border-light-light hover:text-brand-text-dim"}`}>
                {c.label}
              </button>
            ))}
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
          {filtered.map((p) => (
            <div key={p.id} onClick={() => setSelected(p)}
              className="group relative bg-brand-card border border-brand-border-light rounded-sm overflow-hidden cursor-pointer card-lift">
              <div className="aspect-square relative">
                {p.src ? (
                  ...
                  <div className="w-full h-full bg-brand-surface flex flex-col items-center justify-center gap-2">
                    ...
                    <span className="text-xs text-brand-text-dim text-center px-3 leading-tight">{p.title}</span>
                  </div>
                )}
                {/* Overlay */}
                <div className="absolute inset-0 bg-black/70 opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col items-center justify-center gap-1 p-3">
                  ...
                  <span className="text-brand-text-dim text-xs">{p.year}</span>
                </div>
              </div>
              {/* Label */}
              <div className="p-2.5 border-t border-brand-border-light">
                <p className="text-xs text-brand-text-dim truncate">{p.title}</p>
                <p className="text-xs text-brand-text-dim">{p.categoryLabel}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      {selected && (
        <div className="fixed inset-0 z-50 bg-black/90 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setSelected(null)}>
          <div className="bg-brand-surface border border-brand-border-light-light rounded-sm max-w-lg w-full overflow-hidden shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-5">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs text-brand-text-dim">{selected.year}</span>
              </div>
              <h3 className="font-display font-bold text-lg text-brand-text mb-1">{selected.title}</h3>
              <p className="text-sm text-brand-text-dim mb-3">{selected.client}</p>
